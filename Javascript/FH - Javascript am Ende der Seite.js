...
// Section: Produktseitentabs als Toggles darstellen
fhOnReady(function () {
  ...
  function convertNavToToggles(nav) {
    if (!nav || nav.dataset.fhTabsAsToggles === 'true') return;
    ...
    accordion.className = 'fh-tab-toggle-group';
    ...
    toggleCandidates.forEach(function (entry, position) {
      ...
      wrapper.className = 'fh-tab-toggle';
      ...
      trigger.classList.add('fh-tab-toggle__trigger');
      trigger.removeAttribute('data-toggle');
      trigger.removeAttribute('role');
      trigger.removeAttribute('aria-selected');

      const paneId = ensureId(pane, 'fhTabPane-');
      const triggerId = ensureId(trigger, 'fhTabTrigger-');
      ...
      pane.classList.add('fh-tab-toggle__content');
    ...
    nav.dataset.fhTabsAsToggles = 'true';
    nav.remove();
  ...
  if (typeof MutationObserver === 'function') {
    ...
    const observer = new MutationObserver(function (mutations) {
      ...
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          if (shouldCheck || !node || node.nodeType !== 1) return;
...
// End Section: Produktseitentabs als Toggles darstellen
